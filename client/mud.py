...
def loadCharcter(username):
  global newCharcter
  global skills
  print "loading charcter"
  s.send("DATATAG_GETSKILLS " + username)
  try:
    dataIn = 0
    while dataIn == 0:
      dataIn = s.recv(4096)
      if ("DATATAG_GETSKILLS " + username + " ") in dataIn:
        dataIn = dataIn.replace("DATATAG_GETSKILLS " + username + " ","",1)
        exec dataIn in globals()
      else:
        dataIn = 0
  except:
    print "ERROR"
  ...
  if (isInTavern == 1):
    ...
    while (isInTavern == 1):
      ...
      if (m == "leave"):
        ...
        chat = "DATATAG_CHATMESSAGE" + username + ": " + m
        s.send(chat)
        if (linesSoFar < 20):
          linesSoFar = linesSoFar + 1
        else:
          ...
          os.system("cls")
          print Fore.GREEN +  "*--------------------------------------------*\n|Welcome to the tavern - type 'leave' to exit|\n*--------------------------------------------*",
        chat2 = chat.replace("DATATAG_CHATMESSAGE","",1)
        print (pos( 1, linesSoFar ) + chat2 + pos(10, 23 ) )
